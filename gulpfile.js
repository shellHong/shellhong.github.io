...
gulp.task('nav_swig', function() {
  ...
  return g.pipe(gulp.dest(outPath.def));
});
...
gulp.task('del:def', function() {
  return del(outPath.def, {
    force: true,
  });
});
...
//1
gulp.task('swig_task', ['del:dirPath', 'del:dirPath_index', 'del:def'], function() {
